...
Migrations.add({
  ...
  async up() {
    ...
    await peopleCollection.createIndex({ isEmployee: 1 }, { background: false });
    await peopleCollection.createIndex({ isKarkun: 1 }, { background: false });
    await peopleCollection.createIndex({ isVisitor: 1 }, { background: false });
    await peopleCollection.createIndex({ userId: 1 }, { background: false });
    await peopleCollection.createIndex({ 'sharedData.name': 'text' });
