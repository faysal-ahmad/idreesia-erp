...
const NewForm = ({ history }: Props) => {
  ...
  useDynamicBreadcrumbs(['HR', 'Employees', 'New']);
  ...
  const handleFinish = async (values: NewEmployeeFormValues) => {
    ...
    try {
      ...
      const newEmployee = data?.createHrKarkun;
      if (newEmployee?._id) {
        message.success('Employee created', 2);
        history.push(paths.employeeEditFormPath(newEmployee._id));
      }
    } catch (error) {
