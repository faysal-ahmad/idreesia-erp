...
export default class EidReceipts extends Component<ReceiptsProps> {
  ...
  getEidReceipts = (salary: SalaryReceiptRecord) => {
      ...
      <div key={salary._id} className="form-print-view">
        ...
        <AntRow type="flex" justify="start" gutter={10}>
          ...
          <AntCol order={3} style={{ minWidth: '200px' }}>
            <ReceiptItem label="Eid Bonus" value={salary.salary} />
  ...
  render() {
    ...
    // Filter out records where the salary amount is zero.
    const filteredSalaries = filter(
      salariesByIds ?? [],
      (salary: SalaryReceiptRecord) => salary.salary !== 0
    );
