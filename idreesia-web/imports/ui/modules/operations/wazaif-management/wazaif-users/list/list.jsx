...
import { PAGED_OPERATIONS_WAZAIF_USERS } from '../gql';
...
const List = ({ history, location }) => {
  ...
  useEffect(() => {
    dispatch(setBreadcrumbs(['Operations', 'Wazaif Management', 'User Accounts', 'List']));
  }, [location]);

  const { data, loading } = useQuery(PAGED_OPERATIONS_WAZAIF_USERS, {
    variables: {
      filter: queryParams,
    },
  });
  ...
  const { pagedOperationsWazaifUsers } = data;
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'username',
      render: (text, record) => (
        <Link to={paths.wazaifUsersEditFormPath(record._id)}>{text}</Link>
      ),
    },
    {
      title: 'Last Active',
  ...
  return (
    <Table
      rowKey="_id"
      dataSource={pagedOperationsWazaifUsers.data}
      columns={columns}
      bordered
      pagination={false}
      size="small"
      footer={() => (
        <Pagination
          ...
          total={pagedOperationsWazaifUsers.totalResults}
        />
      )}
    />
  );
