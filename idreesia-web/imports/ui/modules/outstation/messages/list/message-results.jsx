...
import { PAGED_OUTSTATION_KARKUN_MESSAGE_RECEPIENTS_BY_RESULT } from '../gql';

const MessageResults = ({ messageId, succeeded }) => {
  const [pageIndex, setPageIndex] = useState(0);
  const [pageSize, setPageSize] = useState(20);
  const { data, loading } = messageId
    ? useQuery(PAGED_OUTSTATION_KARKUN_MESSAGE_RECEPIENTS_BY_RESULT, {
        variables: {
  ...
  return (
    <KarkunsList
      ...
      pageIndex={pageIndex}
      pageSize={pageSize}
      pagedData={data.pagedOutstationKarkunMessageRecepientsByResult}
