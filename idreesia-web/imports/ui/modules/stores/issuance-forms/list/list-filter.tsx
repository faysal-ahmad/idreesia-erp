...
const getIssuanceFilterChips = (
  ...
  const locationId = queryParams.locationId
    ? String(queryParams.locationId)
    : '';
  if (hasFilterValue(locationId)) {
    const location = allLocations.find((item) => item._id === locationId);
    chips.push({
      key: 'locationId',
      label: 'Location',
...
export const IssuanceFilterChips = ({
  refreshPage,
  queryParams,
  allLocations = [],
}: ListFilterProps) => {
  const chips = useMemo(
    () => getIssuanceFilterChips(queryParams, allLocations),
    [queryParams, allLocations]
  );
  ...
  const clearChip = (key: FilterChipKey) => {
    ...
    if (key === 'locationId') {
      refreshPage({ locationId: '', pageIndex: 0 });
      return;
    }
    if (key === 'startDate') {
...
const ListFilter = ({
  ...
  const activeFilterCount = getIssuanceFilterChips(
    queryParams,
    allLocations
  ).length;
