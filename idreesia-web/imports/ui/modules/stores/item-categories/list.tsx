...
import NewForm, { type NewItemCategoryFormValues } from './new-form';
import {
...
const List = () => {
  ...
  const [newForm] = Form.useForm<NewItemCategoryFormValues>();
  ...
  const [createItemCategory, { loading: creating }] = useMutation(
    CREATE_ITEM_CATEGORY,
    {
      refetchQueries: [
        {
  ...
  const handleCreateItemCategory = () =>
    newForm
      ...
      .then(() => {
        message.success('Item category created', 2);
        handleCloseNewForm();
      })
      .catch((error: Error) => {
        if (error?.message) {
  ...
  const columns: any[] = [
    {
      ...
      render: (text: string, record: ItemCategoryRow) => (
        <RouterLink
          to={paths.itemCategoriesEditFormPath(physicalStoreId!, record._id!)}
        >
          {text}
        </RouterLink>
      ),
    ...
    },
    {
      ...
      render: (_text: unknown, record: ItemCategoryRow) => {
        if (record.stockItemCount !== 0) return null;
